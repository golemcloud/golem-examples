...
use clap::*;
use golem_examples::*;
use golem_examples::model::{ExampleName, ExampleParameters, GuestLanguage, GuestLanguageTier, PackageName, TemplateName};
...
pub fn main() {
    ...
    match &command.command {
        Command::New { example: example_name, template_name, package_name } => {
                ...
                Some(example) => {
                    ...
                    match GolemExamples::instantiate(
                        ...
                        ExampleParameters {
                            ...
                            package_name: package_name.clone().unwrap_or(PackageName::from_string("golem:template").unwrap()),
                            target_path: cwd,
                        },
                    ) {
                        Ok(instructions) =>
                        ...
                        Err(err) =>
                            eprintln!("Failed to instantiate template: {err}")
                    }
                }
                None => {
        ...
        Command::ListTemplates { min_tier, language } => {
            GolemExamples::list_all_examples().iter()
                .filter(|example|
                            match language {
